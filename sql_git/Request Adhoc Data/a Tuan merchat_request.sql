...
where (base1.inflow_date between current_date - interval '90' day and current_date - interval '1' day )
...
select 
    ...
    --    ,case when b.parking_fee > 0 then 'Yes' else 'No' end as is_having_pf
       ...
       ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'No driver' then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_no_drivers       
       ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Out of stock' then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_oos       
       ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Shop closed' then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_shop_closed       
