-- DROP TABLE IF EXISTS dev_vnfdbi_opsndrivers.phong_merchant_performance_v2;
-- CREATE TABLE IF NOT EXISTS dev_vnfdbi_opsndrivers.phong_merchant_performance_v2 AS 
with assignment AS
(SELECT * 
...
FROM
        ...
        SELECT   oct.id
                ...
                ,coalesce(date_diff('second',osl.last_incharge_timestamp,osl.last_cancel_timestamp)/cast(60 as double),0) as lt_incharged_cancel
...
where (base1.inflow_date = date'2022-10-10'
       or 
       base1.inflow_date = date'2022-10-20')
...
select 
        -- year(base.inflow_date)*100 + week(base.inflow_date) as year_week
        base.inflow_date           
    --    ,base.inflow_hour 
       ,base.city_name
       ,base.shipper_id
       ,rp.total_online_seconds/cast(3600 as double) as total_online 
       ,rp.total_completed_order 
    --    ,base.merchant_id
    --    ,case when base.merchant_name like '%McDonald%' then '1. VIMs'
    --          when base.merchant_name like '%The Coffee House%' then '1. VIMs'
    --          else '2. Normal' end as merchant_group   
    --    ,base.merchant_name
    --    ,case when b.parking_fee > 0 then 'Yes' else 'No' end as is_having_pf
    --    ,count(distinct base.id)/cast(count(distinct base.inflow_date) as double) as gross_orders
    --    ,count(distinct case when base.is_del = 1 then base.id else null end )/cast(count(distinct base.inflow_date) as double) as net_orders
    --    ,count(distinct case when base.is_canceled = 1 then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders

       ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Out of stock' then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_oos_all
       ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Shop closed' then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_shop_closed_all     
    --    ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Out of stock' and base.shipper_id > 0 then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_oos_have_driver       
    --    ,count(distinct case when base.is_canceled = 1 and cancel_reason = 'Shop closed' and base.shipper_id > 0 then base.id else null end )/cast(count(distinct base.inflow_date) as double) as cancel_orders_shop_closed_have_driver       
       ,count(distinct case when base.is_del = 1 and base.is_asap = 1 then base.id else null end) as total_asap_order
       ,sum(case when base.is_del = 1 and base.is_asap = 1 then lt_completion else null end) as total_completion 
       ,sum(case when base.is_asap = 1 and base.is_canceled = 1 and cancel_reason = 'Out of stock' then base.lt_incharged_cancel else null end) as cancel_lt_incharged_cancel_oos
       ,sum(case when base.is_asap = 1 and base.is_canceled = 1 and cancel_reason = 'Shop closed' then base.lt_incharged_cancel else null end) as cancel_lt_incharged_cancel_shop_closed
    ...
    --    ,sum(ignore_)/cast(sum(assign_turn) as double) as ignore_rate 
    --    ,sum(denied_)/cast(sum(assign_turn) as double) as denied_rate



FROM base 

left join shopeefood.foody_internal_db__shipper_report_daily_tab__reg_daily_s0_live rp on rp.uid = base.shipper_id and date(from_unixtime(rp.report_date - 3600)) = base.inflow_date
-- left join shopeefood.foody_merchant_db__delivery_tab__reg_daily_s0_live b on base.merchant_id = b.restaurant_id
-- LEFT JOIN shopeefood.foody_mart__profile_merchant_master p
