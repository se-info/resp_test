...
where base.report_date between current_date - interval '7' day and current_date - interval '1' day 
...
(select  
       ...
       ,(sum(total_delivered)*1.00/cast((sum(g+h+i)*1.00 + sum(total_delivered)*1.00) as DOUBLE )) as estimate_sla_count_ignore_total  
       ,(sum(total_delivered)*1.00/cast((sum(g+h_1+i)*1.00 + sum(total_delivered)*1.00) as DOUBLE )) as estimate_sla_count_ignore_spxi  
       ,(sum(total_delivered)*1.00/cast((sum(g+h_2+i)*1.00 + sum(total_delivered)*1.00) as DOUBLE )) as estimate_sla_count_ignore_delivery  
...
select 
        ...
        ,city_name
        ,tier
        ,CASE WHEN system_sla < 90 then 1 else 0 end as is_under90_current
        ,CASE WHEN system_sla >= 90 and estimate_sla_count_ignore_total < 0.9 then 1 else 0 end as is_impacted_count_ignore_all
        ,CASE WHEN system_sla >= 90 and estimate_sla_count_ignore_spxi < 0.9 then 1 else 0 end as is_impacted_count_ignore_spxi
        ,CASE WHEN system_sla >= 90 and estimate_sla_count_ignore_delivery < 0.9 then 1 else 0 end as is_impacted_count_ignore_delivery
        ,CASE WHEN system_sla >= 90 and estimate_sla_count_ignore_total_adj_weight < 0.9 then 1 else 0 end as is_impacted_count_ignore_all_and_adj_weight
