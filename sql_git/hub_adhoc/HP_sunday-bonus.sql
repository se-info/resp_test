...
(select 
    ...
    ,case 
        ...
        when (EXTRACT(HOUR from do.inflow_timestamp)*100+ EXTRACT(MINUTE from do.inflow_timestamp) >= 1000 AND EXTRACT(HOUR from do.inflow_timestamp)*100+ EXTRACT(MINUTE from do.inflow_timestamp) < 1300
            or 
        ...
        -- when EXTRACT(HOUR from do.inflow_timestamp)*100+ EXTRACT(MINUTE from do.inflow_timestamp) >= 1800 AND EXTRACT(HOUR from do.inflow_timestamp)*100+ EXTRACT(MINUTE from do.inflow_timestamp) < 2100 then '18pm_21pm'       
...
where date_format(do.inflow_date,'%a') = 'Sun'
and do.inflow_date between current_date  - interval  '7' day and current_date  - interval '1' day
)
...
select 
       ...
       ,total_order
       ,inshift_order
       ,case when shipper_type = 'hub' and is_qualified = 1 and inshift_order >= 20 then 100000
             when shipper_type = 'hub' and is_qualified = 1 and inshift_order >= 12 then 60000            
             when shipper_type = 'hub' and is_qualified = 1 and inshift_order >= 8 then 30000
             
            --  non hub 
             when shipper_type != 'hub' and is_qualified = 1 and total_order >= 20 then 100000
             when shipper_type != 'hub' and is_qualified = 1 and total_order >= 12 then 60000            
             when shipper_type != 'hub' and is_qualified = 1 and total_order >= 8 then 30000
...
(select 
       ...
       ,count(distinct case when driver_payment_policy = 2 then do.order_id else null end) as inshift_order

from final do 

left join kpi_qualified kpi on kpi.shipper_id = do.shipper_id and do.inflow_date = kpi.report_date
...
left join shopeefood.foody_mart__profile_shipper_master pm 
    on do.shipper_id = pm.shipper_id and do.inflow_date = try_cast(pm.grass_date as date)    

where 1 = 1 

and pm.city_name = 'Hai Phong City'


group by 1,2,3,4,5,6,7,8
)

where hour_range != 'Other range'
