...
-- select * from ado where uid = 17980088
...
(
        select  a.shipper_id
               ,a.first_day_in_hub
               ,blt.current_driver_tier as tier_join_hub
...
LEFT JOIN (
        ...
        ) blt ON blt.uid = a.shipper_id AND blt.report_date = a.first_day_in_hub - interval '1' day



left join shopeefood.foody_mart__profile_shipper_master sm on sm.shipper_id = a.shipper_id and sm.grass_date = 'current'

where sm.city_id in (217,218)
)

select  a.*
       ,case when fresh.id is not null then 'Fresh' else 'PT16' end as source 
       ,count(distinct case when b.report_date between a.first_day_in_hub and a.first_day_in_hub + interval '13' day then b.report_date else null end) as total_working_day
       ,count(distinct case when b.report_date between a.first_day_in_hub and a.first_day_in_hub + interval '13' day and is_qualified_kpi = 1 then b.report_date else null end) as total_pass_kpi
       ,sum(case when b.report_date = date'2022-07-07' then 1 else 0 end) as working_7_7
...
left join ado b on a.shipper_id = b.uid


--Input join hub date
where a.first_day_in_hub between date'2022-06-28' and date'2022-07-06'
