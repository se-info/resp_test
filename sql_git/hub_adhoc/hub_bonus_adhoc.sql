...
where is_resume = 1
-- and uid = 4817076
-- and report_date between date'2022-05-30' and date'2022-06-05'
)
...
--- weekly bonus 
(select   a.created_year_week
        ,a.uid
        ,a.shipper_name
        ,a.city_group
        ,concat(cast(reg_v2.shift_hour as varchar),' - ','hour shift') as shift_type
        ...
        ,case 
            ...
            --   New logic updated from 05/06/2022
              when created_year_week >= 202222 and is_work_sun > 0 and kpi_sun = 1
                                              and total_kpi >= 4 
                                              and total_reg = working_day 
                                              then 50000 else 0 end as sunday_bonus
                          
        ,case when total_reg >= 4 and total_reg = total_kpi 
                                  and total_reg = working_day then 1 else 0 end as qualified_kpi
        ,case when total_reg >= 6 
...
(SELECT   
        ...
        ,city_group
        ,reg.total_reg
        ,sum(case when (a.report_date = date'2022-05-29' or a.report_date = date'2022-06-05' or a.report_date = date'2022-06-12' or a.report_date = date'2022-06-19') then a.is_qualified_kpi else null end) as kpi_sun
        ,sum(case when (a.report_date = date'2022-05-29' or a.report_date = date'2022-06-05' or a.report_date = date'2022-06-12' or a.report_date = date'2022-06-19') then a.total_order else null end) as is_work_sun
...
where a.report_date between date'2022-05-23' and date'2022-06-19'
...
(       select year_week
              ,uid 
              ,shift_hour
              ,row_number()over(partition by uid,year_week order by slot_num desc) as rank  
from 
(         select year(date_)*100+week(date_) year_week
                ,uid
                ,shift_hour 
                ,count(distinct slot_id) as slot_num
...
) reg_v2 on reg_v2.uid = a.uid and reg_v2.year_week = a.created_year_week and reg_v2.rank = 1
...
--       ,count(distinct case when weekly_bonus > 0 then uid else null end) as total_eligible_weekly
--       ,count(distinct case when sunday_bonus > 0 then uid else null end) as total_eligible_sunday
--       ,sum(weekly_bonus) as total_cost_weekly
--       ,sum(sunday_bonus) as total_cost_sunday
