...
SELECT week_
     ,uid 
     ,sum(total_order_all) as total_order_all
     ,sum(online_) as online_
     ,sum(is_qualified_kpi) as is_qualified_kpi
     ,count(distinct case when total_order_all > 0 then report_date else null end) as total_working_day 
...
,base.hub_type
,base.shipper_name
,count(case when base.is_inshift = 1 then base.ref_order_id else null end ) as total_order_all
...
(SELECT date_
        ...
        ,uid
        ,shift_hour
        -- ,count(distinct slot_id) as total_register
        ,slot_id
        ,registration_status
        ,row_number()over(partition by uid,year(date_)*100 + week(date_) order by date_ desc) as rank
...
(select
        ...
        -- ,c.shipper_name
        -- ,b.first_day_in_hub
        ,concat(cast(reg_v2.shift_hour as varchar),' - ','hour shift') as last_shift             
        -- ,array_agg(distinct concat(cast(reg.shift_hour as varchar),' - ','hour shift')) as shift_type_combine
        ,(sum(a.online_)*1.00000000/3600)*1.00 as online_inshift
        ,sum(a.is_qualified_kpi) as kpi
        ,sum(a.total_order_all) as total_order_all
        ,sum(reg.total_register) as total_register
        ,sum(total_working_day) as total_working_day
       

--Base raw 
FROM (select year_week
            ,uid 
            -- ,shift_hour
            ,count(case when reg.registration_status != 'OFF' then reg.slot_id else null end) as total_register

        from reg_base reg 
        where 1 = 1 
        and date_ between date'2022-05-23' and date'2022-06-19'
        group by 1,2
        )reg  

--Last shift
left join reg_base reg_v2 on reg_v2.uid = reg.uid and reg_v2.rank = 1 and reg_v2.year_week = reg.year_week

--ADO performance
left join ado a on a.uid = reg.uid and a.week_ = reg.year_week
...
-- LEFT JOIN shopeefood.foody_mart__profile_shipper_master c on c.shipper_id = reg.uid and try_cast(c.grass_date as date) = reg.date_
  ...
  where   1 = 1
-- current_date - interval '75' day and current_date - interval '1' day
...
select year_week
    ...
    --   ,uid 
      ...
      ,sum(online_inshift) as total_online 
      ,sum(kpi) as total_pass_kpi
      ,sum(total_working_day) as total_working_day
