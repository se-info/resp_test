...
(select
          reg.date_
         ,year(reg.date_)*100 + week(reg.date_) as year_week  
         ,concat(cast(reg.shift_hour as varchar),' - ','hour shift') as last_shift  
        -- ,reg.uid
        -- ,c.city_name
        -- ,c.shipper_name
        -- ,b.first_day_in_hub
        ...
        ,count(distinct case when a.is_qualified_kpi = 1 then a.uid else null end) as kpi_
        ,count(distinct case when a.total_order_all > 0 then a.uid else null end) as active
        ,count(distinct case when a.daily_bonus > 0 then a.uid else null end ) as order_bonus 
        -- ,sum(a.is_qualified_kpi)*1.00/sum(total_working_day) as kpi
...
select year_week
    --   ,shift_type_combine
      ...
      ,sum(order_bonus) as order_bonus
