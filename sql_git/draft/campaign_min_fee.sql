...
select   raw.order_id
        ...
        when EXTRACT(HOUR from raw.inflow_timestamp)*100+ EXTRACT(MINUTE from raw.inflow_timestamp) >= 1030 AND EXTRACT(HOUR from raw.inflow_timestamp)*100+ EXTRACT(MINUTE from raw.inflow_timestamp) < 1300 then '1030am_1300pm'
        ...
        when EXTRACT(HOUR from raw.inflow_timestamp)*100+ EXTRACT(MINUTE from raw.inflow_timestamp) >= 1700 AND EXTRACT(HOUR from raw.inflow_timestamp)*100+ EXTRACT(MINUTE from raw.inflow_timestamp) < 2000 then '17pm_20pm'       
...
WHERE 1=1
and coalesce(cast(hm.is_min_fee_guarantee as bigint),0) = 1
and case 
    when base1.hour_range = '1030am_1300pm' and city_name in ('Ha Noi City') then 1
    when base1.hour_range = '1300pm_1330pm' and city_name in ('HCM City','Ha Noi City') then 1 
    when base1.hour_range = '17pm_20pm' and city_name in ('HCM City','Ha Noi City') then 1 
    else 0 end = 1
-- and city_name = 'HN'
