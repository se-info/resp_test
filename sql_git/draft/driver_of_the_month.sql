...
,case when base2.report_date >= date('2021-10-04') and base2.city_group = 'HN' then 1
      ...
      when base2.report_date >= date('2021-10-06') and base2.city_group = 'HP' then 1
...
,base1.uid
,hc.first_day_in_hub
,case when hc.week_join_hub < (base1.created_year_week - 1) then 'Current Driver'
when hc.week_join_hub >= (base1.created_year_week - 1) then 'New Driver'
...
,base.hub_type
,base.city_group
,count(case when base.is_inshift = 1  then base.ref_order_id else null end ) as ado
...
FROM hub
)hub on hc.shipper_id = base1.uid
...
LEFT JOIN shopeefood.foody_internal_db__shipper_hub_income_report_tab__reg_daily_s0_live hub on hc.uid = base2.uid and date(from_unixtime(hc.report_date - 3600)) = base2.report_date
where 1 = 1 )
where is_resume = 1
)
...
select
        ...
        ,c.shipper_name
        ,b.first_day_in_hub
        --,date_diff('day',date(b.first_day_in_hub),a.report_date) as duration_
        --,online_*1.00000000/3600 as online_inshift
        ,case when reg.shift_hour is not null and a.city_name in ('HCM City','Ha Noi City') and reg.shift_hour = 3 and ROUND(reg.start_time/3600,0) >= 18  then concat(cast(reg.shift_hour as varchar ),' - hour shift - C')
              when reg.shift_hour is not null and a.city_name in ('HCM City','Ha Noi City') and reg.shift_hour = 3 and ROUND(reg.start_time/3600,0) >= 17  then concat(cast(reg.shift_hour as varchar ),' - hour shift - S')
              when reg.shift_hour is null and a.city_name in ('HCM City','Ha Noi City') and (c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600 = 3 and ROUND(c.shipper_shift_start_timestamp/3600,0) >= 10  then concat(cast(((c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600) as varchar ),' - hour shift - S')
              when reg.shift_hour is null and a.city_name in ('HCM City','Ha Noi City') and (c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600 = 3 and ROUND(c.shipper_shift_start_timestamp/3600,0) >= 17  then concat(cast(((c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600) as varchar ),' - hour shift - S')
              --when reg.shift_hour is not null then concat(cast(reg.shift_hour as varchar ),' - hour shift')
              ---HP
              when reg.shift_hour is not null and a.city_name = 'Hai Phong City' and reg.shift_hour = 5 and ROUND(reg.start_time/3600,0) >= 15  then concat(cast(reg.shift_hour as varchar ),' - hour shift - C')
              when reg.shift_hour is not null and a.city_name = 'Hai Phong City' and reg.shift_hour = 5 and ROUND(reg.start_time/3600,0) >= 10  then concat(cast(reg.shift_hour as varchar ),' - hour shift - S')
              when reg.shift_hour is null and a.city_name = 'Hai Phong City' and (c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600 = 5 and ROUND(c.shipper_shift_start_timestamp/3600,0) >= 15  then concat(cast(((c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600) as varchar ),' - hour shift - S')
              when reg.shift_hour is null and a.city_name = 'Hai Phong City' and (c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600 = 5 and ROUND(c.shipper_shift_start_timestamp/3600,0) >= 10  then concat(cast(((c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600) as varchar ),' - hour shift - S')
              ---ELSE
              when reg.shift_hour is not null then concat(cast(reg.shift_hour as varchar ),' - hour shift')
              else concat(cast((c.shipper_shift_end_timestamp - c.shipper_shift_start_timestamp)/3600 as varchar),' - hour shift')   end as hub_type
        ,case when reg.registration_status != 'OFF' then reg.registration_status
              else 'N/A' end as registered_
        ,hc.hub_locations
        ,sum(a.is_qualified_kpi) as kpi
        ,try(sum(a.ado)*1.0000/count(distinct a.report_date)) as totla_order
...
)reg  on a.uid = reg.uid and a.report_date = reg.date_

left join driver b on a.uid = b.shipper_id

LEFT JOIN shopeefood.foody_mart__profile_shipper_master c on c.shipper_id = a.uid and try_cast(c.grass_date as date) = a.report_date
left join
(SELECT a1.date_
      ,a1.uid as shipper_id
      ,coalesce(concat(a1.hub_name_1,',',a2.hub_name_2),coalesce(a1.hub_name_1,a2.hub_name_2)) as hub_locations
...
order by date_ desc
)hub on hc.date_ = a.report_date and hc.shipper_id = a.uid
