...
, b as 
       ...
       ,a.report_date 
       ,a.city_name
       ,count(distinct case when policy = 2 then ref_order_code else null end) as inshift_delivered_order
...
(
        SELECT base.report_date
            ...
            --  ,case when si.begin_work_date > 0 then date_diff('second',date_trunc('day',from_unixtime(si.begin_work_date)), date_trunc('day',cast(date(base.report_date) as TIMESTAMP)))/(3600*24) else -1 end as seniority
             --- A7 or A30 with total delivered
             ...
             --- A7 or A30 with inshift delivered
             ,sum(case when base.date_ between date(base.report_date) - interval '29' day and date(base.report_date) then base.total_del_inshift else 0 end) a30_del_inshift
             ,sum(case when base.date_ between date(base.report_date) - interval '6' day and date(base.report_date) then base.total_del_inshift else 0 end) a7_del_inshift
             ,sum(case when base.date_ between date(base.report_date) - interval '1' day and date(base.report_date) then base.total_del_inshift else 0 end) a1_del_inshift

             ---Working day with total delivered
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '29' day and date(base.report_date) )and total_del > 0 then base.date_ else null end) working_day_l30_with_total_order
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '6' day and date(base.report_date)) and total_del > 0 then base.date_ else null end) working_day_l7_with_total_order
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '1' day and date(base.report_date)) and total_del > 0 then base.date_ else null end) working_day_l1_with_total_order

             ---Working day with inshift delivered
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '29' day and date(base.report_date)) and total_del_inshift > 0 then base.date_ else null end) working_day_l30_with_inshift_order
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '6' day and date(base.report_date)) and total_del_inshift > 0 then base.date_ else null end) working_day_l7_with_inshift_order
             ,count(distinct case when (base.date_ between date(base.report_date) - interval '1' day and date(base.report_date)) and total_del_inshift > 0 then base.date_ else null end) working_day_l1_with_inshift_order             
        ...
        FROM
                ...
                Select s1.report_date
                      ...
                      ,coalesce(s2.inshift_delivered_order,0) as total_del_inshift
        ...
        LEFT JOIN shopeefood.foody_mart__profile_shipper_master sm on sm.shipper_id = base.shipper_id and base.report_date = try_cast(sm.grass_date as date)
