...
and cast(json_extract(hc.extra_data,'$.stats.deny_count') as BIGINT) = 0
and cast(json_extract(hc.extra_data,'$.stats.ignore_count') as BIGINT) = 0
and (cast(json_extract(hc.extra_data,'$.stats.online_in_shift') as BIGINT)*1.000/3600)/3 >= 0.9
and array_join(cast(json_extract(extra_data,'$.passed_conditions') as array<int>) ,',') like '%6%'
and cast(json_extract(hc.extra_data,'$.stats.online_peak_hour') as BIGINT)*1.000/3600 >= 1 then 1
...
FROM
(
select *,date(from_unixtime(hub.report_date - 3600)) as date_ ,info.hub_name as hub_name_1
...
info on  info.id = cast(json_extract(hub.extra_data,'$.hub_ids[0]') as bigint)
)a1 
-----
LEFT JOIN 
(select *,date(from_unixtime(hub.report_date - 3600)) as date_ ,info.hub_name as hub_name_2
from shopeefood.foody_internal_db__shipper_hub_income_report_tab__reg_daily_s0_live hub 
---
LEFT JOIN shopeefood.foody_internal_db__shipper_hub_info_tab__reg_daily_s0_live 
info on  info.id = cast(json_extract(hub.extra_data,'$.hub_ids[1]') as bigint)
