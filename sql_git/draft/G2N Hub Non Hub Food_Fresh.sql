...
LEFT JOIN driver_hub on driver_hub.shipper_id = dot.uid and driver_hub.report_date = case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then date(from_unixtime(dot.real_drop_time - 60*60))
                                                                                        when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then date(from_unixtime(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 60*60))
...
FROM base1
LEFT JOIN hub_info on hub_info.id = base1.hub_id
)
,base3 as
(
            SELECT
...
SELECT  sm.shipper_id
        ...
        ,case
            when try_cast(sm.grass_date as date) < date'2021-10-22' then
                ...
                if(ss2.end_time is not null, if((ss2.end_time - ss2.start_time)*1.0000/3600 > 5.00 and (ss2.end_time - ss2.start_time)*1.0000/3600 < 10.00, (ss2.end_time - 28800)*1.0000/3600, ss2.start_time*1.0000/3600)
                    ,if((ss1.end_time - ss1.start_time)*1.0000/3600 > 5.00 and (ss1.end_time - ss1.start_time)*1.0000/3600 < 10.00, (ss1.end_time - 28800)*1.0000/3600, ss1.start_time*1.0000/3600)
                )
        end as start_shift
        ,case
            when try_cast(sm.grass_date as date) < date'2021-10-22' then
                ...
                if(ss2.end_time is not null, ss2.end_time*1.0000/3600, ss1.end_time*1.0000/3600)
        end as end_shift
        ,case
...
SELECT
    ...
    ,report_date
    ,shipper_shift_id
    ,round(start_shift,0) as start_shift
    ,round(end_shift,0) as end_shift
    ,off_weekdays
    ,registration_status
...
SELECT
    ...
    ,hub_dim.hub_type_v3 as hub_type
    ,hub_dim.hub_type_v2
...
LEFT JOIN vnfdbi_opsndrivers.ingest_hub_type_dim_tab AS hub_dim 
    on driver_type.end_shift = cast(hub_dim.end_shift as int) and driver_type.start_shift  = cast(hub_dim.start_shift as int) 
    and driver_type.report_date between cast (hub_dim.start_date as date)  and  coalesce(try_cast(hub_dim.end_date as date) ,current_date)
)
...
SELECT
    ...
    , COUNT(DISTINCT IF(b.oct_cnt_delivered_order_in_shift > 0 AND hub_type_v2 = 'HUB-10', (b.shipper_id, b.grass_date), NULL)) / p.days AS vn_a1_active_hub10_drivers
    , COUNT(DISTINCT IF(b.oct_cnt_delivered_order_in_shift > 0 AND hub_type_v2 = 'HUB-08', (b.shipper_id, b.grass_date), NULL)) / p.days AS vn_a1_active_hub8_drivers
    , COUNT(DISTINCT IF(b.oct_cnt_delivered_order_in_shift > 0 AND hub_type_v2 = 'HUB-05', (b.shipper_id, b.grass_date), NULL)) / p.days AS vn_a1_active_hub5_drivers
    , COUNT(DISTINCT IF(b.oct_cnt_delivered_order_in_shift > 0 AND hub_type_v2 = 'HUB-03', (b.shipper_id, b.grass_date), NULL)) / p.days AS vn_a1_active_hub3_drivers
