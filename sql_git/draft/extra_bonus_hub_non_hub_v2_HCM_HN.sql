...
,delivered_order_from_27may as
(
    select  
        shipper_id
        ,count(distinct order_uid) as delivered_orders
    from driver_order
    where report_date between date '2022-05-27' and date '2022-06-02'
    group by 1
)
,final as
(select 
    ...
    ,o1.city_name
    ,o1.sla
    ,coalesce(o2.delivered_orders,0) as total_27may_02jun_delivered_orders
    ,count(distinct order_uid) as total_order_06_june
from order_on_06_june o1
left join delivered_order_from_27may o2
    on o1.shipper_id = o2.shipper_id
-- where hour_range in ('10am_14pm','16pm_20pm')
where o1.city_name in ('HCM City','Ha Noi City')
group by 1,2,3,4,5
)
select 
    *
from final
where sla >= 0.9
and total_order_06_june >= 10
and total_27may_02jun_delivered_orders = 0
-- having count(distinct order_uid) >= 10
