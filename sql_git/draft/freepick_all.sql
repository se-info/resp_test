...
(SELECT a.order_uid
,a.order_id
,case when a.order_type = 0 then 'NowFood'
    -- when a.order_type = 3 then '1. Food/Market'
    else 'NowShip' end as order_type  
...
where base.date_ between date('2022-01-01') and date(now()) - interval '1' day
...
select created_date
      ...
      ,city_name
      ,source 
      ,count(distinct case when is_free_pick = 1 then order_id else null end) as total_fp_order_del
...
(select 
        ...
        ,a.source 
        ,a.order_id
        ,case when b.order_id is not null then 1 else 0 end as is_free_pick



from all_order a 

left join free_pick b on b.order_id = a.order_id and a.source = b.order_type



where 1 = 1 
and a.created_date between current_date - interval '15' day and current_date - interval '1' day
