...
base2.created_date
    ...
    --   ,base2.city_name
      ...
      ,base2.is_rain
      ,base2.created_hour
      ,count(distinct case when is_late_sla = 1 then base2.uid else null end) total_late_sla_all
    --   ,count(distinct case when is_late_sla = 1 and range_lt_from_sla_to_actual_delivered = '1. Late SLA 0-5 mins' then base2.uid else null end) total_late_sla_0_5min
    --   ,count(distinct case when is_late_sla = 1 and range_lt_from_sla_to_actual_delivered = '2. Late SLA 5-10 mins' then base2.uid else null end) total_late_sla_5_10min
    --   ,count(distinct case when is_late_sla = 1 and range_lt_from_sla_to_actual_delivered = '3. Late SLA 10 - 15 mins' then base2.uid else null end) total_late_sla_10_15min
    --   ,count(distinct case when is_late_sla = 1 and range_lt_from_sla_to_actual_delivered = '4. Late SLA 15+ mins' then base2.uid else null end) total_late_sla_more_than_15min
...
,case when dot.group_id > 0 and coalesce(group_order.order_type,0) = 0 then 1 else 0 end as is_stack
,ogi.group_code
,ogm.group_id
...
--and is_late_sla = 1
and is_asap = 1
and base2.created_date >= date('2021-11-01')
and base2.created_date <= date('2021-11-16')
group by 1,2