...
WHERE DATE(FROM_UNIXTIME(create_time - 3600)) BETWEEN DATE'2021-01-01' AND DATE'2021-12-31'
)
...
SELECT
    ...
    , if(sm1.shipper_status_code <> 1,rea.name,null) as quit_reason 
    , CASE
...
LEFT JOIN
(SELECT *,row_number()over(partition by uid order by update_time desc) as rank
FROM 
shopeefood.foody_internal_db__shipper_quit_request_tab__reg_daily_s0_live) quit on quit.uid = o.shipper_id and quit.rank = 1     
LEFT JOIN shopeefood.foody_internal_db__shipper_quit_request_reason_tab__reg_daily_s0_live rea on rea.id = quit.reason_id    
