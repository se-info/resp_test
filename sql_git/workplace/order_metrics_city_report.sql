...
SELECT 
        ...
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND raw.driver_policy = 2 THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS hub_order,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND raw.group_id > 0 THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS stacked_order,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_no_driver = 1 THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS cnd_order,

        COALESCE(TRY(COUNT(DISTINCT CASE WHEN sub_source = 'delivery' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS gross_order_delivery,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND sub_source = 'delivery' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS net_order_delivery,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND raw.driver_policy = 2 AND sub_source = 'delivery' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS hub_order_delivery,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND raw.group_id > 0 AND sub_source = 'delivery' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS stacked_order_delivery,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_no_driver = 1 AND sub_source = 'delivery' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS cnd_order_delivery,

        COALESCE(TRY(COUNT(DISTINCT CASE WHEN sub_source = 'spxi' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS gross_order_spxi,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND sub_source = 'spxi' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS net_order_spxi,
        0 AS hub_order_spxi,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_del = 1 AND raw.group_id > 0 AND sub_source = 'spxi' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS stacked_order_spxi,
        COALESCE(TRY(COUNT(DISTINCT CASE WHEN is_no_driver = 1 AND sub_source = 'spxi' THEN order_code ELSE NULL END)),0)*1.0000/COUNT(DISTINCT created_date) AS cnd_order_spxi
