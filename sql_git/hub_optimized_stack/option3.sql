...
(select 
        ...
        least(13500*JSON_ARRAY_LENGTH(json_extract(extra_data,'$.route'))*1.00/2
        ,(greatest(3750*(a.distance*1.00/100000)*1,((13500+13500)*0.65)) + 500*JSON_ARRAY_LENGTH(json_extract(extra_data,'$.route'))*1.00/2))
            /(JSON_ARRAY_LENGTH(json_extract(extra_data,'$.route'))*1.00/2) as stacked_per_order_opt1,
        least(
        27000
        ,greatest(13500, 
...
(select 
        ...
        case
        when hp.extra_ship > 0 then hp.total_income - sum(ho.opt2_fee) 
        else 0 end as option2_extra_ship,
        case
        when hp.extra_ship > 0 then hp.total_income - sum(ho.opt3_fee) 
        else 0 end as option3_extra_ship,
        ...
        sum(ho.opt2_fee) as option2_ship_shared,
        sum(ho.opt3_fee) as option3_ship_shared,
...
group by 1,2,3,4,5,6,hp.extra_ship,hp.total_income
)
select  
        ...
        sum(option2_ship_shared) as option2_base,
        sum(option2_extra_ship) as option2_extra,
        sum(option3_ship_shared) as option3_base,
        sum(option3_extra_ship) as option3_extra,
