-- avaiable drivers
with -- avaiable drivers
registered_drivers as
(select 
    -- date(from_unixtime(sr.create_time-3600)) as last_day_of_month
...
--         else 0 end) = 1 ) d
--     on date(from_unixtime(sr.create_time-3600)) = d.report_date
...
-- select 
--     from_unixtime(create_time-3600) ctime
--     ,from_unixtime(update_time-3600) utime
--     ,from_unixtime(meetup_date-3600) mtime
--     ,*
-- from shopeefood.foody_internal_db__shipper_registration_tab__reg_daily_s0_live  sr
-- where date(from_unixtime(sr.create_time-3600)) between date_trunc('month',current_date - interval '1' day) - interval '3' month and current_date - interval '1' day
-- -- and meetup_date is null
-- limit 100
...
where try_cast(grass_date as date) between date_trunc('month',current_date - interval '1' day) - interval '1' month and current_date - interval '1' day
AND sm.shipper_status_code = 1 
group by 1,2
)
...
-- Driver Utilization: net_ado, net ado per driver, stack, % hub coverage, # transacting drivers
,order_base as
(select 
...
where report_date between date_trunc('month',current_date - interval '1' day) - interval '1' month and current_date - interval '1' day
and order_status = 'Delivered'
-- and source in ('NowFood')
group by 1,2
)
...
-- note: current report: % driver utilization based on transcting drivers, but regional report view it as active drivers (diff view)
,active_drivers as
(select 
    slot.grass_date
    ,slot.shipper_id
...
where slot.grass_date between date_trunc('month',current_date - interval '1' day) - interval '1' month and current_date - interval '1' day
group by 1,2,3,4
)
...
where 1=1
and grass_date between date_trunc('month',current_date - interval '1' day) - interval '1' month and current_date - interval '1' day
group by 1,2,3
)
...
where i.grass_date between date_trunc('month',current_date - interval '1' day) - interval '1' month and current_date - interval '1' day
...
(select 
    ...
    ,sum(case when current_driver_tier = 'Hub' then total_orders else 0 end) / count(distinct case when current_driver_tier = 'Hub' then (grass_date,partner_id) else null end)/exchange_rate as hub_ado
    ,sum(case when current_driver_tier != 'Hub' then total_orders else 0 end) / count(distinct case when current_driver_tier != 'Hub' then (grass_date,partner_id) else null end)/exchange_rate as non_hub_ado
...
-- left join drivers_utilization du
--     on ad.report_month = du.report_month
