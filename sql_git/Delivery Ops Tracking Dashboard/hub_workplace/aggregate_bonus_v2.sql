...
(VALUES
(date'2024-06-17',50443900,'5 hour shift-6'),
) 
...
WHERE 1 = 1
AND a.hub_type_original != '1 hour shift'
...
(SELECT a.*
       ,CASE 
             ...
             WHEN hub_type_original = '1 hour shift' then 0
...
-- SELECT coalesce(city_name,'vn') as cities,sum(weekly_bonus_adjusted) as week_,sum(weekly_bonus_orginal) as week_ori
select 
        a.* ,
...
WHERE 1 = 1
GROUP BY 1,2,cp.weekly_paid,cp.sunday_paid
) a 
