...
AND DATE(fa.create_time) = current_date - interval '1' day
-- AND dot.uid = 19399390
)
,final_metrics AS
(SELECT 
        raw.inflow_date
       ...
       ,CASE WHEN is_hub = 1 AND raw.distance < 4 then 12500
             WHEN is_hub = 1 AND raw.distance >= 4 then 15000  
             WHEN is_hub = 0 then GREATEST(12500,3750*distance*surge_rate)
             END AS opt1_hub                                     

       ,CASE WHEN is_hub = 1 AND raw.distance <= 2 then 12500
             WHEN is_hub = 1 AND raw.distance < 4 then  13500
             WHEN is_hub = 1 AND raw.distance >= 4 then 15000  
             WHEN is_hub = 0 then GREATEST(12500,3750*distance*surge_rate)
             END AS opt2_1_hub

       ,CASE WHEN is_hub = 1 AND raw.distance <= 2 then 11500
             WHEN is_hub = 1 AND raw.distance < 4 then  12500
             WHEN is_hub = 1 AND raw.distance >= 4 then 14000  
             WHEN is_hub = 0 then GREATEST(12500,3750*distance*surge_rate)
             END AS opt2_2_hub
       ...
       ,CASE
            WHEN is_hub = 0 THEN GREATEST(12500,(3750*distance*surge_rate))
            ELSE 13500 END AS opt1_non_hub   

       ,CASE
            WHEN is_hub = 0 AND distance <= 2 THEN 12500
            WHEN is_hub = 0 AND distance > 2 THEN GREATEST(13500,(3750*distance*surge_rate))
            ELSE 13500 END AS opt2_non_hub 

       ,CASE
            WHEN is_hub = 0 AND distance <= 2 THEN 12500
            WHEN is_hub = 0 AND distance > 2 THEN GREATEST(13500,(3650*distance*surge_rate))
            ELSE 13500 END AS opt3_non_hub 

       ,CASE
            WHEN is_hub = 0 THEN GREATEST(11650,(3750*distance*surge_rate))
            ELSE 13500 END AS opt4_non_hub  
...
(SELECT 
      ...
      ,SUM(CASE WHEN is_hub = 1 then opt1_hub ELSE 0 END) AS opt1_hub
      ,SUM(CASE WHEN is_hub = 1 then opt2_1_hub ELSE 0 END) AS opt2_1_hub
      ,SUM(CASE WHEN is_hub = 1 then opt2_2_hub ELSE 0 END) AS opt2_2_hub
      ,SUM(CASE WHEN is_hub = 1 then opt3_hub ELSE 0 END) AS opt3_hub
      ,SUM(CASE
               WHEN is_hub = 0 AND group_id = 0 then opt1_non_hub
               WHEN is_hub = 0 AND group_id > 0 then group_fee ELSE 0 END) AS opt1_non_hub
      ,SUM(CASE
               WHEN is_hub = 0 AND group_id = 0 then opt2_non_hub
               WHEN is_hub = 0 AND group_id > 0 then group_fee ELSE 0 END) AS opt2_non_hub
      ,SUM(CASE
               WHEN is_hub = 0 AND group_id = 0 then opt3_non_hub
               WHEN is_hub = 0 AND group_id > 0 then group_fee ELSE 0 END) AS opt3_non_hub
      ,SUM(CASE
               WHEN is_hub = 0 AND group_id = 0 then opt4_non_hub
               WHEN is_hub = 0 AND group_id > 0 then group_fee ELSE 0 END) AS opt4_non_hub                              
...
SELECT
       ...
       ,SUM(CASE WHEN driver_type != 'Non Hub' AND current_base > 0 THEN current_base ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_current_hub
       ,SUM(CASE WHEN driver_type = 'Non Hub' AND current_base > 0 THEN current_base ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_current_non_hub
       ,SUM(CASE WHEN driver_type != 'Non Hub' AND total_order_hub > 0 THEN opt1_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_hub_opt1
       ,SUM(CASE WHEN driver_type != 'Non Hub' AND total_order_hub > 0 THEN opt2_1_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_hub_opt2_1
       ,SUM(CASE WHEN driver_type != 'Non Hub' AND total_order_hub > 0 THEN opt2_2_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_hub_opt2_2
       ,SUM(CASE WHEN driver_type != 'Non Hub' AND total_order_hub > 0 THEN opt3_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_hub_opt3
       ...
       ,SUM(CASE WHEN opt3_non_hub > 0 THEN opt3_non_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_non_hub_opt3
       ,SUM(CASE WHEN opt4_non_hub > 0 THEN opt4_non_hub ELSE 0 END)/CAST(fx.exchange_rate AS DOUBLE) AS shipping_non_hub_opt4


FROM summary s 

LEFT JOIN mp_order.dim_exchange_rate__reg_s0_live fx 
    on fx.grass_date = s.inflow_date
    and fx.currency = 'VND'
