...
FROM
    (
(
SELECT sequence(date'2022-08-01',current_date - interval '1' day) bar)
...
inner join params_date p on hi.report_date between p.start_date and p.end_date

where hi.dummy_city_id != 999
and p.period_group = '1. Daily'

and (case when p.period_group = '1. Daily' then date_format(p.start_date,'%a') else null end) = 'Sun'
