...
,base1.city_group
,base1.inflow_hour
,count(distinct base1.uid)/cast(count(distinct inflow_date) as double) as net_orders
...
,cast(count(distinct case when peak_mode_name = 'Peak 1 Mode' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_peak1_orders
,cast(count(distinct case when peak_mode_name = 'Peak 2 Mode' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_peak2_orders
,cast(count(distinct case when peak_mode_name = 'Peak 3 Mode' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_peak3_orders
,cast(count(distinct case when peak_mode_name = 'Normal' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_normal_orders
,cast(count(distinct case when distance_range = '1. 0-3km' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_under_3km_orders
,cast(count(distinct case when distance_range = '2. 3km+' then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_above_3km_orders
,cast(sum(case when distance_range = '1. 0-3km' then base1.distance else null end) as double)/cast(count(distinct inflow_date) as double) as under_3km_distance
,cast(sum(case when distance_range = '2. 3km+' then base1.distance else null end) as double)/cast(count(distinct inflow_date) as double) as above_3km_distance
,count(distinct case when is_hub_order = 0 then base1.uid else null end)/cast(count(distinct inflow_date) as double) total_non_hub_order
,count(distinct case when is_hub_order = 1 then base1.uid else null end)/cast(count(distinct inflow_date) as double) total_hub_order
...
,cast(count(distinct case when distance_range = '1. 0-3km' and is_hub_order = 0 then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_non_hub_under_3km_orders
,cast(count(distinct case when distance_range = '2. 3km+' and is_hub_order = 0 then base1.uid else null end) as double)/cast(count(distinct inflow_date) as double) as net_non_hub_above_3km_distance
...
left JOIN shopeefood.foody_partner_db__driver_order_tab__reg_daily_s0_live dot on dot.ref_order_id = all.id and dot.ref_order_category = 0 
...
where (date(inflow_timestamp) = date'2022-08-08')
