     ...
     (
      ...
      FROM
        ((
         SELECT
           asb.id
         , "json_extract"(asb.processing_info, '$.ds_group_response.grouped_jobs') group_jobs
         , CAST("json_extract"(asb.processing_info, '$.order_category') AS bigint) order_category
         , city.name_en city_name
         , asb.city_id
         , asb.grass_date
         FROM
           (shopeefood.foody_partner_archive_db__order_assign_shipper_batch_processing_log_tab_di asb
         ...
         AND asb.processing_info LIKE '%ds_grouping%'
...
UNION ALL    SELECT
     '2. Order grouped assign driver' section
   , order_source
   , date_
   , city_group
   , city_name
   , total_order_in_group
   , "count"(DISTINCT order_id) kpi
   FROM
     group_assign
   GROUP BY 1, 2, 3, 4, 5, 6
UNION ALL    SELECT
     '3. Order grouped driver accept' section
   , order_source
   , date_
   , city_group
   , city_name
   ...
   GROUP BY 1, 2, 3, 4, 5, 6
UNION ALL    SELECT
     (CASE WHEN (total_order_in_group_actual_del = 0) THEN '3.1. Order grouped - complete 0 order' WHEN (total_order_in_group_actual_del < total_order_in_group) THEN '3.2. Order grouped - complete >=1 order & deny/cancel >=1 order' ELSE '3.3. Order grouped - complete all orders' END) section
   , order_source
   , date_
   , city_group
   , city_name
...
where m1.date_ >= date'2024-02-01'
