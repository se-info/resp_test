...
LEFT JOIN dev_vnfdbi_opsndrivers.phong_hub_driver_metrics hm 
    on hm.uid = dot.uid 
    and hm.date_ = DATE(FROM_UNIXTIME(dot.real_drop_time - 3600))
    and FROM_UNIXTIME(dot.real_drop_time - 3600) BETWEEN hm.start_shift_time AND hm.end_shift_time
...
AND DATE(FROM_UNIXTIME(dot.real_drop_time - 3600)) between date'2023-06-26' and date'2023-07-02'
...
(SELECT
        ...
        ,raw.city_name
        ,raw.hub_type
        ,CASE WHEN raw.hub_type in ('10 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 30 AND raw.kpi = 1 THEN (13500*30)- SUM(raw.shipping_current) 
              WHEN raw.hub_type in ('8 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 25 AND raw.kpi = 1 THEN (13500*25)- SUM(raw.shipping_current) ELSE 0 END AS extra_current

        ,CASE WHEN raw.hub_type in ('10 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 30 AND raw.kpi = 1 THEN (13500*30)- SUM(raw.shipping_current) 
              WHEN raw.hub_type in ('8 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 25 AND raw.kpi = 1 THEN (13500*25)- SUM(raw.opt1) ELSE 0 END AS extra_opt1

        ,CASE WHEN raw.hub_type in ('10 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 30 AND raw.kpi = 1 THEN (13500*30)- SUM(raw.shipping_current) 
              WHEN raw.hub_type in ('8 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 25 AND raw.kpi = 1 THEN (13500*25)- SUM(raw.opt2) ELSE 0 END AS extra_opt2

        ,CASE WHEN raw.hub_type in ('10 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 30 AND raw.kpi = 1 THEN (13500*30)- SUM(raw.shipping_current) 
              WHEN raw.hub_type in ('8 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 25 AND raw.kpi = 1 THEN (13500*25)- SUM(raw.opt3) ELSE 0 END AS extra_opt3

        ,CASE WHEN raw.hub_type in ('10 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 30 AND raw.kpi = 1 THEN (13500*30)- SUM(raw.shipping_current) 
              WHEN raw.hub_type in ('8 hour shift') AND raw.city_name IN ('HCM City','Ha Noi City') AND COUNT(DISTINCT raw.ref_order_code) < 25 AND raw.kpi = 1 THEN (13500*25)- SUM(raw.opt4) ELSE 0 END AS extra_opt4
...
SELECT
        ...
        ,(SUM(total_cost_opt1 + extra_opt1)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS total_cost_opt1
        ,(SUM(total_cost_opt2 + extra_opt2)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS total_cost_opt2
        ,(SUM(total_cost_opt3 + extra_opt3)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS total_cost_opt3
        ,(SUM(total_cost_opt4 + extra_opt4)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS total_cost_opt4

        ,(SUM(extra_opt1)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS extra_opt1
        ,(SUM(extra_opt2)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS extra_opt2
        ,(SUM(extra_opt3)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS extra_opt3
        ,(SUM(extra_opt4)/CAST(COUNT(DISTINCT report_date) AS DOUBLE))/CAST(ex.exchange_rate AS DOUBLE) AS extra_opt4
...
where report_date = date'2023-07-02'
