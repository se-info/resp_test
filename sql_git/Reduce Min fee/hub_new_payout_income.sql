...
(select 
        ...
        hi.shift_start_time/3600 as start_shift,
...
from vnfdbi_opsndrivers.snp_foody_shipper_income_tab s 

where 1 = 1 
and s.date_ between date'2023-10-01' and date'2023-10-31'
and s.hub_weekly_bonus > 0 
)
,auto_pay as 
(select 
        report_date,
...
(select 
        a.report_date,
        a.shipper_id,
        max_by(a.shift_hour,a.shift_hour) as shift_hour,
        sum(a.ship_shared + extra_ship + daily_bonus + coalesce(wb.hub_weekly_bonus,0)) as current_base,
        sum(a.ship_shared_opt1 + extra_opt1 + daily_bonus + coalesce(wb.hub_weekly_bonus,0)) as opt1_base,
        sum(a.ship_shared_opt2 + extra_opt2 + daily_bonus + coalesce(wb.hub_weekly_bonus,0)) as opt2_base,
        sum(a.ship_shared_opt3 + extra_opt3 + daily_bonus + coalesce(wb.hub_weekly_bonus,0)) as opt3_base,
        sum(a.ship_shared_opt4 + extra_opt4 + daily_bonus + coalesce(wb.hub_weekly_bonus,0)) as opt4_base
        -- sum(a.daily_bonus) as daily_bonus,
...
left join weekly_bonus wb 
    on wb.date_ = a.report_date
    and wb.partner_id = a.shipper_id


where 1 = 1 
group by 1,2, wb.hub_weekly_bonus
)
select
        date_trunc('month',report_date) as month_,
        shift_hour,
        sum(current_base)/cast(count(shipper_id) as double) as avg_income_current,
        sum(opt1_base)/cast(count(shipper_id) as double) as avg_income_opt1,
        sum(opt2_base)/cast(count(shipper_id) as double) as avg_income_opt2,
        sum(opt3_base)/cast(count(shipper_id) as double) as avg_income_opt3,
        sum(opt4_base)/cast(count(shipper_id) as double) as avg_income_opt4
