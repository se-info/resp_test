...
(SELECT 
       ...
       ,GREATEST(12000,3750*dotet.surge_rate_single*dot.distance) as non_hub_opt1 
--        ,GREATEST(13500,dotet.unit_fee_single*1*dot.distance) as non_hub_opt2
--        ,GREATEST(IF(dot.distance <=2,12000,13500),dotet.unit_fee_single*dotet.surge_rate_single*dot.distance) as non_hub_opt3_4
       ...
       ,CASE 
        ...
        ,GREATEST(
                12000,
                ROUND(3750 * (dot.distance) *
                      dotet.surge_rate_single
                     )
        ) AS single_opt1
       ,CASE 
            WHEN ROW_NUMBER()OVER(PARTITION BY dot.group_id ORDER BY dot.picked_timestamp ASC) = 1 
                THEN GREATEST(12000,ROUND(3750 * (dot.distance) * dotet.surge_rate_single))
            ELSE 0
            END AS fee_1_opt1
        ,CASE 
...
and (case 
       when delivered_timestamp is not null then DATE(delivered_timestamp)
       else created_date end) >= date'2024-03-01'
and dot.order_status in ('Delivered','Quit','Returned')
and dot.shipper_id > 0 
and dot.order_type = 0
and dot.city_name in ('Dak Lak','Thanh Hoa','Binh Dinh','Binh Thuan')
)
,group_info as 
(select  
        group_id,
        ...
        GREATEST(
            ...
            ),LEAST(SUM(single_opt1),ROUND(MAX(ROUND(group_distance,1))*3750*MAX(surge_rate))))
         WHEN MAX(rank_order) > 2 THEN GREATEST(SUM(single_opt1)*IF(group_category=0,1,0.7),LEAST(SUM(single_opt1),MAX(ROUND(group_distance,1))*3750*MAX(surge_rate)))  
...
(select 
        ...
        shift_hour,
        case 
        when oi.city_name in ('HCM City','Ha Noi City','Da Nang City','Dak Lak','Thanh Hoa','Binh Dinh','Binh Thuan') then oi.city_name
