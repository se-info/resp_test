...
and date(create_time) between date'2024-01-15' and date'2024-01-21'
group by 1,2,3,4
) s 
...
-- select * from s where cnt_ignored_group > 0
select  
        city_id,
        is_new_group, 
        sum(cnt_assign_group)/cast(count(distinct created) as double) as cnt_assign_group,
        -- if counting assign need to change total_order_in_group => total_order_in_group at final due to the with group (1) is need to be updated shipping_fee
        avg(case when total_order_in_group > 1 then re else null end) as avg_re,
        avg(case when total_order_in_group > 1 then group_distance else null end) as avg_group_distance,
        avg(case when total_order_in_group > 1 then group_fee else null end) as avg_group_fee,
        1 - sum(case when total_order_in_group > 1 then group_fee else null end)
        /cast(sum(case when total_order_in_group > 1 then single_fee else null end) as double) as saving_single

from s 
-- where city_id = 217
where 1 = 1 
and cnt_ignored_group > 0
group by 1,2
