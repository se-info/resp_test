...
select 
    grass_date
    ,case 
        when city_name_full in ('HCM City') then 'HCM'
        when city_name_full in ('Ha Noi City') then 'HN'
        when city_name_full in ('Da Nang City') then 'DN'
        else 'OTH' end as city_group
    ,case 
        when distance <= 3 then '1.<= 3km'
...
select 
    ...
    ,case 
        when distance_grp <= 3 then '1.<= 3km'
        when distance_grp > 3 then '2.++3km'
        else null end as distance_range
    ,'stack' as type_
