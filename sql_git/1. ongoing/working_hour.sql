...
(SELECT 
        ...
        end_time,
        shipper_id,
        LEAST(SUM(work_by_hour*1.0000/3600),1) AS work_hour
...
SELECT  
        ds.created,
        ds.uid,
        SUM(ds.online_by_hour*1.00/3600) AS online_hour,
        SUM(dw.work_hour) AS work_hour

FROM driver_ops_driver_supply_tab ds 

LEFT JOIN driver_work dw ON dw.shipper_id = ds.uid 
                         AND dw.start_time = ds.start_time


WHERE ds.uid = 42156627
AND ds.created = date'2024-04-13'
GROUP BY 1,2
