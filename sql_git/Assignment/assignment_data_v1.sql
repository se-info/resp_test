-- DROP TABLE IF EXISTS dev_vnfdbi_opsndrivers.phong_raw_group_assignment_v1;
-- CREATE TABLE IF NOT EXISTS dev_vnfdbi_opsndrivers.phong_raw_group_assignment_v1
-- AS 
with raw_date as 
(SELECT
...
FROM
    (
(
SELECT sequence(current_date - interval '45' day,current_date - interval '1' day) bar)
...
AND date(from_unixtime(a.create_time - 60*60)) BETWEEN DATE_TRUNC('month', current_date - interval '1' day) - interval '1' month AND current_date - interval '1' day
...
FROM
    ...
    SELECT dot.uid as shipper_id
            ...
            ,case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then date(from_unixtime(dot.real_drop_time - 60*60))
                  when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then date(from_unixtime(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 60*60))
                  else date(from_unixtime(dot.submitted_time- 60*60)) end as inflow_date
            ,HOUR(case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then from_unixtime(dot.real_drop_time - 60*60)
                  when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then from_unixtime(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 60*60)
                  else from_unixtime(dot.submitted_time- 60*60) end) as inflow_hour
            ,date(from_unixtime(dot.submitted_time- 60*60)) as created_date 
            ...
            ,case when li.experiment_group in (3,4) then 1 ELSE 0 end as is_auto_accepted
            ,case when li.experiment_group in (7,8) then 1 ELSE 0 end as is_auto_accepted_continuous_assign        
    ...
    LEFT JOIN last_incharge li on li.order_id = dot.ref_order_id and dot.ref_order_category = li.order_type
...
select  
        * 
       ,coalesce(_nowfood_single_ado/cast(nowfood_ado as double),0) as pct_sinlge_food  
       ,coalesce(_nowfood_stack_ado/cast(nowfood_ado as double),0) as pct_stack_food   
       ,coalesce(_nowfood_group_ado/cast(nowfood_ado as double),0) as pct_group_food
       --  
    --    ,coalesce(_nowship_single_ado/cast(nowship_ado as double),0) as pct_sinlge_ship  
    --    ,coalesce(_nowship_stack_ado/cast(nowship_ado as double),0) as pct_stack_ship   
    --    ,coalesce(_nowship_group_ado/cast(nowship_ado as double),0) as pct_group_ship

       ,coalesce(_nowfood_late_eta/cast(nowfood_ado as double),0) as pct_late_eta_food
    --    ,coalesce(_nowship_late_eta/cast(nowship_ado as double),0) as pct_late_eta_ship
FROM  
(SELECT
     p.period_group
    ,p.period
    ...
    -- , p.days AS days            
    , total_order_in_group
    , SUM(IF(service = 'Food/Market', delivered_orders, 0)) /cast(p.days as double) AS nowfood_ado
    , SUM(IF(service = 'Food/Market' AND assignment_type = 'Group Order', delivered_orders, 0)) /cast(p.days as double) AS _nowfood_group_ado
    , SUM(IF(service = 'Food/Market' AND assignment_type = 'Stack Order', delivered_orders, 0)) /cast(p.days as double) AS _nowfood_stack_ado
    , SUM(IF(service = 'Food/Market' AND assignment_type = 'Single Order', delivered_orders, 0)) /cast(p.days as double) AS _nowfood_single_ado
    -- , SUM(IF(service = 'SPX Instant', delivered_orders, 0)) /cast(p.days as double) AS nowship_ado
    -- , SUM(IF(service = 'SPX Instant' AND assignment_type = 'Group Order', delivered_orders, 0)) /cast(p.days as double) AS _nowship_group_ado
    -- , SUM(IF(service = 'SPX Instant' AND assignment_type = 'Stack Order', delivered_orders, 0)) /cast(p.days as double) AS _nowship_stack_ado
    -- , SUM(IF(service = 'SPX Instant' AND assignment_type = 'Single Order', delivered_orders, 0)) /cast(p.days as double) AS _nowship_single_ado
    , SUM(IF(is_late_eta = 1 AND service = 'Food/Market',delivered_orders,0 )) /cast(p.days as double) AS _nowfood_late_eta
    -- , SUM(IF(is_late_eta = 1 AND service = 'SPX Instant',delivered_orders,0 )) /cast(p.days as double) AS _nowship_late_eta  
    , SUM(IF(is_hub_qualified = 1 AND service = 'Food/Market',delivered_orders,0 )) /cast(p.days as double) AS _nowfood_hub_qualified
    , SUM(IF(is_inshift_order = 1 AND service = 'Food/Market',delivered_orders,0 )) /cast(p.days as double) AS _nowfood_inshift_orders
    , SUM(IF(is_auto_accepted = 1 AND service = 'Food/Market',delivered_orders,0 )) /cast(p.days as double) AS _nowfood_auto_accept_orders
    , SUM(IF(is_auto_accepted_continuous_assign = 1 AND service = 'Food/Market',delivered_orders,0 )) /cast(p.days as double) AS _nowfood_ca_orders
