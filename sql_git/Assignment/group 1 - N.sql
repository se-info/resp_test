...
FROM
        ...
        SELECT base.shipper_id
              ...
              ,date_diff('second',base.created_timestamp ,base.confirm_timestamp)*1.0000/60 as lt_submit_to_confirm -- t_confirm            
              ,date_diff('second',base.confirm_timestamp,base.last_picked_timestamp)*1.0000/60 as lt_merchant_prep -- t_prep 
              ,date_diff('second',base.created_timestamp ,base.last_incharge_timestamp)*1.0000/60 as lt_submit_to_incharge -- t_assign
              ,date_diff('second',base.last_incharge_timestamp ,base.last_picked_timestamp)*1.0000/60 as lt_incharge_to_pick -- t_pickup               
              ,date_diff('second',base.last_picked_timestamp ,base.last_delivered_timestamp)*1.0000/60 as lt_pick_to_del -- t_drop_off
        ...
        FROM
                    ...
                    and date(case when dot.is_asap = 0 then fa.first_auto_assign_timestamp else from_unixtime(dot.submitted_time- 60*60) end) between current_date - interval '90' day and current_date - interval '1' day
        ...
        and (base.created_date = date'2022-09-09'
            or 
            base.created_date = date'2022-08-08'
            )
...
,total_order_in_group
,is_fp_maxhandle
,count(distinct case when is_valid_submit_to_del = 1 and is_late_eta_max = 1 then order_id else null end) total_late_eta_max
,count(distinct case when is_valid_submit_to_del = 1 and is_late_sla = 1 then order_id else null end) total_late_sla
,count(distinct case when is_valid_submit_to_del = 1 then order_id else null end) total_del
,sum(case when is_valid_submit_to_del = 1 then lt_completion else 0 end) sum_lt_completion

,sum(case when is_valid_submit_to_del = 1 and is_valid_lt_submit_to_incharge = 1 then lt_submit_to_incharge else 0 end) sum_lt_submit_to_incharge
,sum(case when is_valid_submit_to_del = 1 and is_valid_lt_incharge_to_pick = 1 then lt_incharge_to_pick else 0 end) sum_lt_incharge_to_pick
,sum(case when is_valid_submit_to_del = 1 and is_valid_lt_pick_to_del = 1 then lt_pick_to_del else 0 end) sum_lt_pick_to_del
...
left join shopeefood.foody_mart__profile_merchant_master mm on mm.merchant_id = base1.merchant_id and try_cast(mm.grass_date as date) = base1.created_date

-- where is_group_order = 1 

where city_group in('1. HCM','2. HN')
