...
where date_ between current_date - interval '45' day and current_date - interval '1' day    
)
,cpo_base as
(
    select 
    ...
    -- Include bad weather fee
    -- CPO all
    ,(sum(dr_cost_base_usd) + sum(dr_cost_surge_usd) + sum(dr_cost_bonus_usd) + sum(dr_cost_bw_fee_usd))/count(distinct order_id) as driver_cpo_base_surge_bonus_all_bwf
    ,(sum(dr_cost_bw_fee_usd) + sum(dr_cost_late_night_usd) + sum(dr_cost_holiday_fee))/count(distinct order_id) as driver_cpo_etc_all
    ...
    +sum(case when delivered_by = 'hub' then dr_cost_surge_usd else 0 end) 
    +sum(case when delivered_by = 'hub' then dr_cost_bonus_usd else 0 end)
    +sum(case when delivered_by = 'hub' then dr_cost_bw_fee_usd else 0 end) 
    ) / count(case when delivered_by = 'hub' then order_id else null end) as driver_cpo_base_surge_bonus_hub_bwf
    ...
    +sum(case when delivered_by != 'hub' then dr_cost_bonus_usd else 0 end)
    +sum(case when delivered_by != 'hub' then dr_cost_bw_fee_usd else 0 end)
    ) / count(case when delivered_by != 'hub' then order_id else null end) as driver_cpo_base_surge_bonus_non_hub_bwf
...
select 
    ...
    city_name
    ,cast(year_week as varchar) as period
    ,delivered_hour
    ,source
    -- orders
    ,count(distinct order_id) as total_orders
...
select 
    ...
    ,t.delivered_by
    ,t.metrics
    ,concat(cpo.city_name,'_',cast(cpo.delivered_hour as varchar),'_',t.delivered_by) as key_map
    ,t.cpo_base_surge_bonus
    ,t.cpo_base_surge_bonus_bwf
...
,array[driver_cpo_base_surge_bonus_all_bwf,driver_cpo_base_surge_bonus_hub_bwf,driver_cpo_base_surge_bonus_non_hub_bwf]
) t(delivered_by,metrics,cpo_base_surge_bonus,cpo_base_surge_bonus_bwf)
