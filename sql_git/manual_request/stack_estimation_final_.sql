...
(select 
    ...
    ,raw.distance_grp
    ,raw.exchange_rate
    ,cast(json_extract(re.extra_data,'$.re') as bigint) re_stack_system
    ,try(round(distance_all,1)*1.00/round(distance_grp,0)) as re_stack_manual
...
(select 
    f.*
    ,case when re_stack_manual > 1.05 then 1 
...
(
    select 
        ...
        ,bf.delivered_by
        ,case when delivered_by = 'hub' then 1 else s.stack_adjust end as stack_adjust
        ,s.stack_adjust as stack_adjust_v2
...
-- select * from bill_fee_raw where order_id in (337366921,337360272)
select 
    grass_date
    ,distance_range
    ,count(distinct case when is_stack_group_order = 2 then order_id else null end ) as total_stack_orders_current
    ...
    ,count(distinct case when is_stack_group_order = 2 and stack_adjust = 1 then order_id else null end ) as total_stack_orders_est_nonhub_only
    ,sum(case when stack_adjust = 1 then total_after_stack_fee_v1 else b_total_before_stack_fee end)/exchange_rate as option4
    ,count(distinct case when is_stack_group_order = 2 and stack_adjust_v2 = 1 then order_id else null end ) as total_stack_orders_est
    ,sum(case when stack_adjust_v2 = 1 then total_after_stack_fee_v1 else b_total_before_stack_fee end)/exchange_rate as option4_1
