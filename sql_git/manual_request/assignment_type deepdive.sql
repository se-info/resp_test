...
, sum(all.total_del) total_del
, sum(CASE WHEN (all.is_stack = 1) THEN all.total_del ELSE null END) total_stack_del
, sum(CASE WHEN (all.is_fp_maxhandle = 1) THEN all.total_del ELSE null END) total_fp_del
, sum(CASE WHEN (all.is_group_order = 1) THEN all.total_del ELSE null END) total_group_del
FROM
  (
with free_pick as 
(
...
where created_date between date'2022-09-12' and date'2022-09-18'
