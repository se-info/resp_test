...
(select 
      ...
      ,(coalesce(bf.distance_all,bf.distance)*1.00/cast(json_extract(re.extra_data,'$.re') as bigint))*cast(json_extract(re.extra_data,'$.ship_fee_info.surge_rate') as bigint)*cast(json_extract(re.extra_data,'$.ship_fee_info.per_km') as bigint) as spf_by_stacked_distance_v2
      ,bf.date_
      ...
      ,case when is_stack_group_order = 2 then coalesce(bf.distance_all,bf.distance)*1.00/cast(json_extract(re.extra_data,'$.re') as bigint) else coalesce(bf.distance_all,bf.distance) end as new_stack_distance
...
(select 
        ...
        ,spf_by_stacked_distance_v2
        ,total_before_stack_fee
        ,min_group_shipping_fee

        --Calculate by new distance and current stack fee formula
        ,case when is_stack_group_order = 2  and rank  = 1 and delivered_by != 'hub'  
              then greatest(spf_by_stacked_distance_v2 + extra_fee,min_group_shipping_fee) 
              else 0 
              end as new_stack_fee_v1     
        --Calculate by new distance and new stack fee formula
        ,case when is_stack_group_order = 2  and rank  = 1 and delivered_by != 'hub'  
            --   then greatest( cast(total_before_stack_fee as bigint),greatest(cast(total_ship_fee_ab as bigint),cast(max_a_b as bigint)) ) 
              then greatest(least(spf_by_stacked_distance_v2 + extra_fee,total_before_stack_fee),min_group_shipping_fee)      
              else 0 
        ...
        ,sum(distinct case when is_stack_group_order = 2 and rank = 1 then extra_fee else null end) as extra_fee
...
select date_
    ...
    --   saving
      ...
      ,(sum(sum_single_fee - new_stack_fee_v1))*1.00/exchange_rate as estimate_saving_stack_v1
...
and date_ between date'2022-06-15' and date'2022-07-14'
