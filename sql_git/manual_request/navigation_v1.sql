...
(
    select city_group
        ...
        ,delivered_date
        ,ref_order_code
        ,count(case when ability_navigation is not null and delivery_distance_meter <= 1000 then ability_navigation else null end) ability_back_to_hub
...
select  city_group 
        ,metrics        
        ,case when ability_back_to_hub >= 10 then '10 +'
              when ability_back_to_hub > 0 and ability_back_to_hub is not null then cast(ability_back_to_hub as varchar )
              else 'Non eligible' end as ability_assign_back_to_hub
        ,count(distinct ref_order_code)*1.0000/count(distinct delivered_date) as total_order_drop_out_hub
