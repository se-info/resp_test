...
FROM
    (SELECT
        ...
        , last_drop_hub
    FROM hub_onboard
    WHERE first_join_hub BETWEEN DATE'2022-08-27' AND DATE'2022-09-08'
    )
GROUP BY 1
)
, grass_date AS
...
FROM
    ((SELECT sequence(DATE'2022-08-27', DATE'2022-09-21') bar)
CROSS JOIN
...
FROM
    ...
    AND case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then date(FROM_UNIXTIME(dot.real_drop_time - 3600))
            when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then date(FROM_UNIXTIME(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 3600))
            else date(FROM_UNIXTIME(dot.submitted_time- 3600)) end BETWEEN DATE'2022-08-27' AND DATE'2022-09-21'
    )
GROUP BY 1,2
)
, all_ AS
(SELECT
    ...
    , sm.shipper_name
    , sm.city_name
    , IF(ho.shipper_id IS NOT NULL, 1, 0) AS is_onboard_before_aug27
    , h.first_join_hub
...
LEFT JOIN (SELECT shipper_id FROM hub_onboard WHERE first_join_hub < DATE'2022-08-27' GROUP BY 1) ho ON h.shipper_id = ho.shipper_id
...
(select 
    ...
    ,all_.is_onboard_before_aug27
    ,all_.first_join_hub
    ,all_.end_bonus_date
    ,sum(case when report_date = date '2022-09-09' then is_eligible_day else null end) as is_eligile_09_sep
    ,sum(is_eligible_day) as total_eligible_days
    ,sum(case when report_date = date '2022-09-09' and inshift_delivered_orders > 0 then 1 else 0 end) is_have_order_09_sep
...
select 
    ...
    ,f.is_onboard_before_aug27
    ,f.first_join_hub
    ,f.end_bonus_date
    ,f.total_eligible_days
    ,f.is_have_order_09_sep
    ,f.list_hub_worked
    ,f.is_eligile_09_sep
    ,case when fresh.id is not null then 1 else 0 end as is_fresh
    ,case
        when total_eligible_days >=6 and is_have_order_09_sep >= 1 and is_onboard_before_aug27 = 0 and is_eligile_09_sep = 1  and driver_tier_before_join_hub in ('T1','T2','T3') then 100000
        when total_eligible_days >=6 and is_have_order_09_sep >= 1 and is_onboard_before_aug27 = 0 and is_eligile_09_sep = 1 and driver_tier_before_join_hub in ('T4','T5') then 250000
        when total_eligible_days >=6 and is_have_order_09_sep >= 1 and is_onboard_before_aug27 = 0 and is_eligile_09_sep = 1 and driver_tier_before_join_hub is null and f.city_name in ('Hai Phong City') then 100000
        else 0 end as bonus_scheme

from final f

left join vnfdbi_opsndrivers.foody_hub_fresh_driver_data fresh on f.shipper_id = cast(fresh.id as bigint)
