...
(SELECT        dot.uid as shipper_id
              ...
              ,case when coalesce(oct.sub_total,0) >= 200000 then 1 else 0 end as is_over_200sub_total
        ...
        and (case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then date(from_unixtime(dot.real_drop_time - 60*60))
                    when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then date(from_unixtime(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 60*60))
                    else date(from_unixtime(dot.submitted_time- 60*60)) end) = date'2022-12-25' 
...
select  raw.report_date
      ...
      ,rp.completed_rate/cast(100 as double) as driver_sla
      ,rp.total_work_distance/cast(1000 as double) as working_distance
      ,count(distinct raw.order_code) as total_delivered
      ,count(distinct case when raw.report_hour_min between 0 and 1959 then raw.order_code else null end) as total_order_0_1959
      ,SUM(case when raw.report_hour_min between 0 and 1959 then raw.total_item else null end) as total_item_0_1959      
      ,sum(is_over_200sub_total) as delivery_order_over_200_sub_total
...
(SELECT        dot.uid as shipper_id
        ...
        where 1 = 1 
        and dot.order_status = 400
        and (date(from_unixtime(dot.real_drop_time - 3600)) = date'2022-12-25') or (date(from_unixtime(dot.real_drop_time - 3600)) = date'2022-12-24')
...
select 
      ...
      ,case 
       ...
       ,count(distinct raw.order_code ) as total_orders 
