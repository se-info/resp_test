...
(SELECT        dot.uid as shipper_id
              ...
              ,coalesce(oct.sub_total,0) as sub_total
              ,coalesce(oct.total_item,0) as total_item 
              ,coalesce(oct.total_value,0) as total_value
              ,case when coalesce(oct.sub_total,0) > 700000 then 1 else 0 end as is_over_700sub_total
              ,case when coalesce(oct.total_item,0) > 10 then 1 else 0 end as is_over_10_item
        ...
        left join 
            (
            select id,cast(json_extract(extra_data,'$.total_item') as bigint) as total_item, total_amount/cast(100 as double) as total_value,foody_service_id,sub_total/cast(100 as double) as sub_total
            from
        ...
        and (case when dot.order_status in (400,401,405) and dot.real_drop_time > 0 then date(from_unixtime(dot.real_drop_time - 60*60))
                    when dot.order_status in (402,403,404) and cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) > 0 then date(from_unixtime(cast(json_extract(dotet.order_data,'$.delivery.status_update_time') as bigint) - 60*60))
                    else date(from_unixtime(dot.submitted_time- 60*60)) end) = date'2022-11-20' 
                    -- between date'2022-11-18' and date'2022-11-20'
)
...
select  raw.report_date
    ...
    --   ,raw.food_service
      ...
      ,count(distinct case when source = 'order_delivery' then raw.order_code else null end) as total_delivery
      ,count(distinct case when source != 'order_delivery' then raw.order_code else null end) as total_spxi
      ,sum(case when source = 'order_delivery' then is_over_10_item else null end) as delivery_order_over_10_item
      ,sum(case when source = 'order_delivery' then is_over_700sub_total else null end) as delivery_order_over_700_sub_total
      ,array_agg(case when is_over_10_item = 1 and source = 'order_delivery' then order_code end) as ext_order_over_10_item
      ,array_agg(case when is_over_700sub_total = 1  and source = 'order_delivery' then order_code end) as ext_order_over_700_sub_total
