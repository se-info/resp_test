...
-- where o.report_date between date'2023-12-01' and date'2024-01-31'
...
(
    select 
        report_month
        -- ,case when city_group in('HCM','HN','DN') then city_group else 'OTH' end as metrics
        -- ,distance_range
        -- ,is_bw
        ...
        ,avg(case when is_stack_group_order = 1 then driver_waiting_time_at_mex else null end) as avg_driver_waiting_time_mex_stack
        ,avg(case when is_stack_group_order = 1 then driver_waiting_time_at_user else null end) as avg_driver_waiting_time_user_stack
    ...
    -- select 
    --     report_month
    --     -- ,case when city_group in('HCM','HN','DN') then city_group else 'OTH' end as city_group
    --     ,distance_range as metrics
    --     ,count(distinct ref_order_id)/cast(count(distinct report_date) as double) as asap_order
    --     ,avg(payment_time) as avg_payment_time
    --     ,avg(assignment_time) as avg_assignment_time
    --     ,avg(driver_to_store) as avg_driver_to_store
...
select 
    ...
    ,tp.avg_driver_waiting_time_mex_stack
    ,tp.avg_driver_waiting_time_user_stack
