DROP TABLE IF EXISTS dev_vnfdbi_opsndrivers.temp_driver_list_tab;       
CREATE TABLE IF NOT EXISTS dev_vnfdbi_opsndrivers.temp_driver_list_tab AS 
WITH raw AS
(SELECT 
     raw.report_date
    ,raw.shipper_id
    ...
    ,ROUND((raw.total_order*1.00/CAST((COALESCE(sa.denied,0) + COALESCE(sa.ignore_opt1,0) + raw.total_order) AS DOUBLE) )*100,2) AS sla_opt1                                         
    ,ROUND((raw.total_order*1.00/CAST((COALESCE(sa.denied,0) + COALESCE(sa.ignore_opt1,0)*0.5 + raw.total_order) AS DOUBLE) )*100,2) AS sla_opt2                                                        
    ,ROUND((raw.total_order*1.00/CAST((COALESCE(sa.denied,0) + COALESCE(sa.ignore_opt2,0) + raw.total_order) AS DOUBLE) )*100,2) AS sla_opt3                                       
...
LEFT JOIN (SELECT 
                ...
                ,COUNT(DISTINCT CASE WHEN status IN (8,9,17,18) THEN (order_id,create_time) ELSE NULL END) AS ignore_opt1                                         
...
WHERE raw.report_date BETWEEN date'2023-11-01' and date'2023-11-30'
AND raw.total_order > 0 
)
,driver_impact_list as 
(select 
...
select 
        ...
        l.city_tier,
        dp.turn_pass_sla,
        row_number()over(order by dp.turn_pass_sla desc) as rank_
...
from driver_impact_list
) l 

left join (select 
                shipper_id,
                count(distinct case when sla_rate >= 90 then report_date else null end)/cast(count(distinct report_date) as double) as turn_pass_sla

        from driver_ops_driver_performance_tab 
        where total_order > 0 
        group by 1) dp 
    on dp.shipper_id = l.shipper_id